...
const obstacles=[];
// Photographic cutout of the supplied subject and guitar, anchored on the lawn.
// Rotate only around Y so the figure remains readable as visitors circle the house.
const performerTexture=new THREE.TextureLoader().load(`${import.meta.env.BASE_URL}violonista.webp`);
performerTexture.colorSpace=THREE.SRGBColorSpace;
const performer=new THREE.Mesh(new THREE.PlaneGeometry(1.36,2.04),new THREE.MeshBasicMaterial({map:performerTexture,transparent:true,alphaTest:.04,depthWrite:false,side:THREE.DoubleSide,toneMapped:false}));
